...
def verify_wikipedia(text: str) -> FactualCheck:
    ...
    if cache_key in RAG_CACHE.get("wikipedia", {}):
        cached = RAG_CACHE["wikipedia"][cache_key]
...
async def analyze_article(
    ...
    if feature_engine and ensemble_model and len(parsed_text.strip()) > 0:
        try:
            ...
            #
            # Two conditions flag insufficient data:
            # 1. Very few absolute matches (< 4 recognized words) — not enough signal
            # 2. Low coverage ratio (< 30%) — input is out-of-domain
            insufficient_data = matched_terms < 4 or vocab_coverage < 0.3
            if insufficient_data:
                # Scale confidence by how much content the model actually understood
                signal_strength = min(matched_terms / 4.0, vocab_coverage / 0.3)
    ...
    # -- RAG EVIDENCE INTEGRATION (6-SOURCE CONSENSUS) --
    ...
    # Fact-checker verdicts carry extra weight
    ...
    if len(explainability) < 5 and len(parsed_text.strip()) > 0:
        ...
        if vocab_coverage < 0.3 and len(explainability) < 5:
