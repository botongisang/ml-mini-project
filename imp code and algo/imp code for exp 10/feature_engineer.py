...
Pipeline B: GloVe average embeddings (100d) — captures SEMANTICS that TF-IDF misses
Pipeline C: Stylometric features (17d)   — captures writing style, detects AI text

Total feature vector: ~267 dimensions (dense, manageable, works with LIME/SHAP)
"""
...
def _load_glove():
    """Load GloVe model via gensim lazily."""
    global _glove_model, _glove_load_success
    if _glove_model is None and config.ENABLE_GLOVE:
        print("[FEATURES] Loading GloVe embeddings (first time may download ~128MB)...")
        try:
            import gensim.downloader as api
            _glove_model = api.load(config.GLOVE_MODEL_NAME)
            _glove_load_success = True
            print(f"[FEATURES] GloVe loaded: {len(_glove_model)} word vectors, {config.GLOVE_DIM}d")
        except Exception as e:
            warnings.warn(f"Failed to load GloVe: {e}. Skipping GloVe features.")
            _glove_load_success = False
            return None
    return _glove_model
...
class TruthLensFeatureEngine:
    ...
    def fit(self, processed_texts, cleaned_texts=None, raw_texts=None):
        ...
        # Pipeline B: GloVe (no fitting needed — pretrained)
        if config.ENABLE_GLOVE:
            _load_glove()

        # Track whether GloVe actually loaded (not just whether config enabled it)
        self.glove_actually_loaded = _glove_load_success and config.ENABLE_GLOVE
        ...
        self.is_fitted = True
        self.trained_with_glove = self.glove_actually_loaded
    ...
    def _transform_all(self, processed_texts, cleaned_texts, raw_texts):
        ...
        # Pipeline B: GloVe embeddings
        # Use training-time flag if available to ensure dimension match
        use_glove = getattr(self, "trained_with_glove", None)
        if use_glove is None:
            use_glove = config.ENABLE_GLOVE
        if use_glove:
            texts_for_glove = cleaned_texts if cleaned_texts is not None else processed_texts
            glove_features = self._extract_glove(texts_for_glove)
            features_list.append(glove_features)
    ...
    def _extract_glove(self, texts):
        """
        Extract document-level GloVe embeddings by averaging word vectors.

        Unlike TF-IDF, GloVe captures SEMANTIC meaning:
        - "good" and "great" have similar vectors
        - "effective" and "ineffective" have different vectors
        """
        glove = _load_glove()
        if glove is None:
            return np.zeros((len(texts), config.GLOVE_DIM))

        doc_vectors = []
        for text in tqdm(texts, desc="GloVe embeddings", leave=False):
            if not isinstance(text, str) or len(text.strip()) == 0:
                doc_vectors.append(np.zeros(config.GLOVE_DIM))
                continue
    ...
    def _build_feature_names(self):
        ...
        # GloVe dimension names
        use_glove = getattr(self, "trained_with_glove", config.ENABLE_GLOVE)
        if use_glove:
            names += [f"glove_{i}" for i in range(config.GLOVE_DIM)]
    ...
    def save(self, path=None):
        ...
        with open(path, "wb") as f:
            pickle.dump({
                ...
                "trained_with_glove": getattr(self, "trained_with_glove", config.ENABLE_GLOVE),
    ...
    def load(self, path=None):
        ...
        if "trained_with_glove" in data:
            self.trained_with_glove = data["trained_with_glove"]
        else:
            # Infer from scaler dimensions for models saved before this field existed
            self.trained_with_glove = expected_features > (config.SVD_COMPONENTS + NUM_STYLOMETRIC + 5)
            print(f"[FEATURES] Inferred trained_with_glove={self.trained_with_glove} from {expected_features} features")
        ...
        if "num_stylometric" in data:
            ...
            glove_dims = config.GLOVE_DIM if self.trained_with_glove else 0
            self._num_stylometric = expected - svd_dims - glove_dims
